...
export default async function getAll(req: any, res: any) {
  ...
  try {
    await collectionMalik.createIndex({
      terjemah: "text",
      kitab_nama: "text",
      arab: "text",
      koleksi: "text",
    ...
    const countAllData = await collectionMalik.countDocuments();
    ...
    if (!containsOnlyNumbers(sc) && req.query.search != undefined) {
      ...
      countSearchResult = await collectionMalik.aggregate(agg2).toArray();
    ...
    if (containsOnlyNumbers(sc)) {
      ...
      countSearchResult = await collectionMalik.aggregate(agss).toArray();
