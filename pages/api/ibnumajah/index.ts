const { collectionIbnumajah } = require("../../../config/database");
...
export default async function getAll(req: any, res: any) {
  ...
  try {
    await collectionIbnumajah.createIndex({
      terjemah: "text",
      kitab_nama: "text",
      arab: "text",
      koleksi: "text",
    ...
    const countAllData = await collectionIbnumajah.countDocuments();
    ...
    if (!containsOnlyNumbers(sc) && req.query.search != undefined) {
      ...
      countSearchResult = await collectionIbnumajah.aggregate(agg2).toArray();

      if (countSearchResult[0] != undefined) {
        return collectionIbnumajah
          .aggregate(agg1)
    ...
    if (containsOnlyNumbers(sc)) {
      ...
      countSearchResult = await collectionIbnumajah.aggregate(agss).toArray();
