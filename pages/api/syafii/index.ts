const { collectionSyafii } = require("../../../config/database");
...
export default async function getAll(
  ...
  try {
    await collectionSyafii.createIndex({
      terjemah: "text",
      kitab_nama: "text",
      arab: "text",
      koleksi: "text",
    ...
    const countAllData = await collectionSyafii.countDocuments();
    ...
    if (!containsOnlyNumbers(sc) && req.query.search != undefined) {
      ...
      countSearchResult = await collectionSyafii.aggregate(agg2).toArray();

      if (countSearchResult[0] != undefined) {
        return collectionSyafii
          .aggregate(agg1)
    ...
    if (containsOnlyNumbers(sc)) {
      ...
      countSearchResult = await collectionSyafii.aggregate(agss).toArray();
