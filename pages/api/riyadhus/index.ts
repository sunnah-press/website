const { collectionRiyadhus } = require("../../../config/database");
...
export default async function getAll(req: any, res: any) {
  ...
  try {
    await collectionRiyadhus.createIndex({
      terjemah: "text",
      kitab_nama: "text",
      arab: "text",
      koleksi: "text",
    ...
    const countAllData = await collectionRiyadhus.countDocuments();
    ...
    if (!containsOnlyNumbers(sc) && req.query.search != undefined) {
      ...
      countSearchResult = await collectionRiyadhus.aggregate(agg2).toArray();

      if (countSearchResult[0] != undefined) {
        return collectionRiyadhus
          .aggregate(agg1)
    ...
    if (containsOnlyNumbers(sc)) {
      ...
      countSearchResult = await collectionRiyadhus.aggregate(agss).toArray();
